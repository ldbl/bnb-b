...
class SignalValidator:
    ...
    def save_signal(self, signal_data: Dict) -> bool:
        ...
        try:
            ...
            fib_level = "N/A"
            if (
                signal_data.get("fibonacci_analysis")
                and "fibonacci_signal" in signal_data["fibonacci_analysis"]
            ):
            ...
            tail_strength = "N/A"
            if (
                signal_data.get("weekly_tails_analysis")
                and "tails_signal" in signal_data["weekly_tails_analysis"]
            ):
            ...
            logger.info(
                f"Сигнал записан: {signal_type} на {signal_date.strftime('%Y-%m-%d')} при ${signal_price:,.2f}"
            )
            return True
    ...
    def get_accuracy_stats(self, lookback_days: int = 30) -> Dict[str, any]:
        ...
        try:
            ...
            logger.info(
                f"Статистика за точността: {accuracy:.1f}% ({successful_signals}/{total_signals})"
            )
            return stats
        ...
        try:
            with open(output_file, "w", encoding="utf-8") as f:
                ...
                validated_signals = len(
                    self.results_df[pd.notna(self.results_df["validation_date"])]
                )
