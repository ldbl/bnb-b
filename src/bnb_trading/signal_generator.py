...
class SignalGenerator:
    ...
    def __init__(self, config: Dict[str, Any]) -> None:
        ...
        logger.info(
            f"Приоритет: Fibonacci={self.fibonacci_weight}, Weekly Tails={self.weekly_tails_weight}"
        )
    ...
    def generate_signal(self, daily_df: pd.DataFrame, weekly_df: pd.DataFrame) -> Dict[str, Any]:
        ...
        try:
            ...
            if "error" in optimal_levels_analysis:
                logger.warning(
                    f"Optimal Levels анализ неуспешен: {optimal_levels_analysis['error']}"
                )
            ...
            elliott_wave_analysis = self.elliott_wave_analyzer.analyze_elliott_wave(
                daily_df, weekly_df
            )
            ...
            rsi_values = (
                daily_with_indicators["RSI"].tolist()
                if "RSI" in daily_with_indicators.columns
                else []
            )
            macd_values = (
                daily_with_indicators["MACD"].tolist()
            ...
            if (
                self.multi_timeframe_analyzer.enabled
                ...
                try:
                    ...
                    multi_timeframe_result = (
                        self.multi_timeframe_analyzer.analyze_timeframe_alignment(
                    ...
                    if multi_timeframe_result and isinstance(multi_timeframe_result, dict):
                        ...
                        logger.warning(
                            "Multi-timeframe анализ върна невалиден резултат, използваме fallback"
                        )
                        multi_timeframe_analysis = {
            ...
            if multi_timeframe_analysis:
                ...
                final_signal["confidence"] = max(
                    0.0, min(1.0, final_signal["confidence"] + confidence_bonus)
                )
                ...
                logger.info(
                    f"Multi-timeframe analysis applied: {multi_timeframe_analysis.get('overall_alignment', 'UNKNOWN')} | Bonus: {confidence_bonus:+.2f}"
                )

            logger.info(
                f"Сигнал генериран: {final_signal['signal']} (увереност: {final_signal['confidence']:.2f})"
            )
    ...
    def _combine_signals(
        ...
        try:
            ...
            if fib_analysis and "fibonacci_signal" in fib_analysis:
                ...
                if fib_signal["signal"] != "HOLD":
                    ...
                    signal_reasons.append(
                        f"Fibonacci: {fib_signal['reason']} (сила: {fib_signal['strength']:.2f})"
                    )
            ...
            if tails_analysis and "tails_signal" in tails_analysis:
                ...
                if tails_signal["signal"] != "HOLD":
                    ...
                    if macd_conflict:
                        ...
                        signal_reasons.append(
                            f"Weekly Tails: {tails_signal['reason']} (сила: {tails_signal['strength']:.2f}) - тегло намалено поради MACD конфликт"
                        )
                    else:
                        adjusted_weight = self.weekly_tails_weight
                        signal_reasons.append(
                            f"Weekly Tails: {tails_signal['reason']} (сила: {tails_signal['strength']:.2f})"
                        )
                ...
                if "error" not in ma_analysis:
                    ...
                    if crossover.get("signal") and crossover["signal"] != "NONE":
                        ...
                        if ma_signal != "HOLD":
                            ...
                            if (
                                weekly_tails_signal
                                and weekly_tails_signal["signal"] == "SHORT"
                                and weekly_tails_signal["strength"] > 0.8
                            ):
                                adjusted_weight = base_weight * 0.6  # Намаляваме с 40%
                                signal_reasons.append(
                                    f"Moving Averages: {crossover['signal']} → {ma_signal} ({crossover['confidence']:.0f}%) - тегло намалено поради силен Weekly SHORT"
                                )
                            else:
                                adjusted_weight = base_weight
                                signal_reasons.append(
                                    f"Moving Averages: {crossover['signal']} → {ma_signal} ({crossover['confidence']:.0f}%)"
                                )
            ...
            if (
                self.config.get("long_signals", {}).get("ema_confirmation", False)
                and "moving_averages_analysis" in locals()
                ...
                if (
                    ...
                    signal_reasons.append(
                        f"✅ EMA ПОТВЪРЖДЕНИЕ: {crossover['signal']} ({crossover['confidence']:.0f}%) - +{ema_confidence_bonus:.2f} confidence за LONG"
                    )
                    long_signal_confirmed = True
                    logger.info(
                        f"EMA потвърждение за LONG сигнал: +{ema_confidence_bonus} confidence"
                    )

            # PHASE 2: BNB Burn Enhancement за LONG сигнали
            burn_enhanced = False
            if (
                self.config.get("long_signals", {}).get("burn_enhancement", False)
                and daily_df is not None
            ...
            if (
                fib_analysis
                and "fibonacci_levels" in fib_analysis
                and (
                    signal_scores["LONG"] > signal_scores["SHORT"]
                    ...
                    if support_levels:
                        ...
                        if closest_support:
                            ...
                            stop_loss_recommendation = {
                                ...
                                "risk_pct": ((current_price - stop_loss_price) / current_price)
                                * 100,
                    ...
                    if resistance_levels:
                        ...
                        if closest_resistance:
                            ...
                            stop_loss_recommendation = {
                                ...
                                "risk_pct": ((stop_loss_price - current_price) / current_price)
                                * 100,
            ...
            if "daily_df" in locals() and daily_df is not None and not daily_df.empty:
                ...
                if "ATH_Proximity_Score" in daily_df.columns:
                    ...
                    if ath_distance_pct > 10.0:  # Далеч от rolling ATH - блокираме SHORT
                        ...
                        signal_reasons.append(
                            f"SHORT BLOCKED by rolling ATH proximity: {ath_distance_pct:.1f}% под ATH (твърде далеч)"
                        )
                        logger.info(
                            f"SHORT blocked by rolling ATH proximity: {ath_distance_pct:.1f}% distance from ATH"
                        )
            ...
            if "daily_df" in locals() and daily_df is not None and not daily_df.empty:
                ...
                if trend_analysis and "combined_trend" in trend_analysis:
                    ...
                    trend_strength = self._score_to_strength(
                        combined_trend.get("combined_strength", 0)
                    )
                ...
                if "ATH_Distance_Pct" in daily_df.columns:
                    ...
                    if ath_distance > 15.0 and trend_direction in ["STRONG_UPTREND"]:
                        ...
                        signal_reasons.append(
                            f"SHORT moderately weakened: {ath_distance:.1f}% from ATH + strong uptrend"
                        )
                        logger.info(
                            f"SHORT moderately weakened: {ath_distance:.1f}% from ATH + strong uptrend"
                        )
            ...
            if total_weight == 0:
                ...
                if final_signal == "LONG":
                    ...
                    if (
                        ...
                        if tail_strength > 0.8:  # Strong tail = potential reversal
                            ...
                            quality_factors.append(
                                f"Long Tail Reversal Pattern ({tail_strength:.2f})"
                            )
                    ...
                    if multi_timeframe_analysis:
                        ...
                        if overall_alignment == "STRONG" and alignment_score >= 0.8:
                            ...
                            warning_factors.append(
                                f"Poor timeframe alignment ({alignment_score:.1%})"
                            )
                    ...
                    if enhanced_confidence < long_confidence_threshold:
                        ...
                        confidence = enhanced_confidence
                        quality_summary = (
                            f"HIGH QUALITY LONG (confidence: {enhanced_confidence:.2f})"
                        )
                        if quality_factors:
                ...
                if (
                    ...
                    if trend_filter_applied["blocked"]:
                        ...
                        confidence = 0.5
                        signal_reasons.append(
                            f"SHORT BLOCKED by trend filter: {trend_filter_applied['reason']}"
                        )
                ...
                if final_signal == "SHORT" and tails_analysis and fib_analysis:
                    fib_resistance_filter_applied = (
                        self._apply_fibonacci_resistance_filter_for_short(
                            tails_analysis, fib_analysis
                        )
                    )
                    if fib_resistance_filter_applied["blocked"]:
                        ...
                        confidence = 0.4
                        signal_reasons.append(
                            f"SHORT BLOCKED by Fibonacci resistance filter: {fib_resistance_filter_applied['reason']}"
                        )
                ...
                if self.fib_tail_required:
                    ...
                    if not has_fib_or_tail and confidence < (
                        self.confidence_threshold * 0.7
                    ):  # Намалено от 0.8 до 0.56
                        final_signal = "HOLD"
                        confidence = 0.5
            ...
            if final_signal == "LONG" and daily_df is not None and weekly_df is not None:
                ...
                if divergence_analysis:
                    ...
                    if divergence_long_result["bonus"] != 0.0:
                        ...
                        long_enhancements_reasons.append(
                            f"Divergence LONG: {divergence_long_result['reason']}"
                        )
                ...
                if market_regime_long_result["bonus"] != 0.0:
                    ...
                    long_enhancements_reasons.append(
                        f"Market Regime LONG: {market_regime_long_result['reason']}"
                    )

                # Phase 3: Advanced LONG Signal Confirmations
                # Phase 3.1: Ichimoku Cloud Confirmation
                if ichimoku_analysis and self.config.get("long_signals", {}).get(
                    "ichimoku_confirmation_enabled", False
                ):
                    ichimoku_long_result = self._check_ichimoku_confirmation_for_long(
                        ichimoku_analysis, current_price
                    )
                    if ichimoku_long_result["bonus"] != 0.0:
                        long_enhancements_bonus += ichimoku_long_result["bonus"]
                        long_enhancements_reasons.append(
                            f"Ichimoku LONG: {ichimoku_long_result['reason']}"
                        )

                # Phase 3.2: Sentiment Confirmation
                if sentiment_analysis and self.config.get("long_signals", {}).get(
                    "sentiment_confirmation_enabled", False
                ):
                    sentiment_long_result = self._check_sentiment_confirmation_for_long(
                        sentiment_analysis
                    )
                    if sentiment_long_result["bonus"] != 0.0:
                        long_enhancements_bonus += sentiment_long_result["bonus"]
                        long_enhancements_reasons.append(
                            f"Sentiment LONG: {sentiment_long_result['reason']}"
                        )
                ...
                if whale_analysis and self.config.get("long_signals", {}).get(
                    ...
                    if whale_long_result["bonus"] != 0.0:
                        ...
                        long_enhancements_reasons.append(
                            f"Whale LONG: {whale_long_result['reason']}"
                        )
                ...
                if self.config.get("long_signals", {}).get(
                    ...
                    if patterns_long_result["bonus"] != 0.0:
                        ...
                        long_enhancements_reasons.append(
                            f"Patterns LONG: {patterns_long_result['reason']}"
                        )
                ...
                if elliott_wave_analysis and self.config.get("long_signals", {}).get(
                    ...
                    if elliott_long_result["bonus"] != 0.0:
                        ...
                        long_enhancements_reasons.append(
                            f"Elliott LONG: {elliott_long_result['reason']}"
                        )
                ...
                if optimal_levels_analysis and self.config.get("long_signals", {}).get(
                    ...
                    if levels_long_result["bonus"] != 0.0:
                        ...
                        long_enhancements_reasons.append(
                            f"Optimal Levels LONG: {levels_long_result['reason']}"
                        )
                ...
                if long_enhancements_bonus != 0.0:
                    ...
                    logger.info(
                        f"LONG Enhancement: {long_enhancements_reasons}, bonus: {long_enhancements_bonus:.2f}, confidence: {old_confidence:.2f} → {confidence:.2f}"
                    )
    ...
    def _apply_fibonacci_resistance_filter_for_short(
        ...
        try:
            if not tails_analysis or not fib_analysis:
                return {"blocked": False, "reason": "Недостатъчно данни за анализ"}

            tails_signal = tails_analysis.get("tails_signal", {})
            if tails_signal.get("signal") != "SHORT":
                return {"blocked": False, "reason": "Не е SHORT сигнал"}

            # Взимаме Fibonacci нива
            fib_levels = fib_analysis.get("fibonacci_levels", {})
            if not fib_levels:
                return {"blocked": False, "reason": "Няма Fibonacci нива"}
            ...
            for resistance_price in resistance_levels:
                ...
                if price_distance_pct <= proximity_threshold:
                    logger.info(
                        f"SHORT allowed: Опашка близо до resistance {resistance_price:.2f} "
    ...
    def _check_volume_confirmation_for_short(self, daily_df: pd.DataFrame) -> Dict[str, any]:
        ...
        try:
            if daily_df is None or daily_df.empty:
                return {"confirmed": False, "reason": "Няма данни за volume анализ"}
            ...
            if not volume_confirmation:
                return {"confirmed": True, "reason": "Volume confirmation изключен"}

            # Проверяваме дали има Volume колона
            if "Volume" not in daily_df.columns and "volume" not in daily_df.columns:
                return {"confirmed": False, "reason": "Няма Volume данни"}
    ...
    def _check_multi_timeframe_alignment_for_short(self, trend_analysis: Dict) -> Dict[str, any]:
        ...
        try:
            if not trend_analysis or "error" in trend_analysis:
                return {"aligned": False, "reason": "Няма валиден trend анализ"}
            ...
            if not alignment_enabled:
                return {"aligned": True, "reason": "Multi-timeframe alignment изключен"}
            ...
            regime_criteria = {
                "strong_bull": (
                    ...
                    or (
                        ...
                        and weekly_direction in ["UPTREND", "BULLISH"]
                    )
                ),
                "weak_bull": (
                    (
                        weekly_direction in ["UPTREND", "BULLISH"]
                        and weekly_strength in ["MODERATE", "WEAK"]
                    )
                    or (
                        daily_direction in ["UPTREND", "BULLISH"]
                        and weekly_direction in ["UPTREND", "BULLISH"]
                ...
                "range": (
                    range_status == "RANGE"
                    or (volatility_ratio < 0.8 and abs(range_position - 0.5) < 0.2)
                ),
                "bear": (
                    (weekly_direction in ["DOWNTREND", "BEARISH"])
                    or (
                        daily_direction in ["DOWNTREND", "BEARISH"]
                        and daily_strength in ["STRONG", "MODERATE"]
                    )
                ),
            }
    ...
    def _check_market_regime_for_short(
        self,
    ...
    def _check_volume_confirmation_for_long(self, daily_df: pd.DataFrame) -> Dict[str, any]:
        ...
        try:
            ...
            # Проверяваме дали има Volume колона
            if "Volume" not in daily_df.columns and "volume" not in daily_df.columns:
                return {"bonus": 0.0, "reason": "Няма Volume данни", "confirmed": False}
        ...
        try:
            ...
            if fib_analysis and "fibonacci_signal" in fib_analysis:
                ...
                if fib_signal.get("signal") == "SHORT":
                    ...
                    score_breakdown["fibonacci_reason"] = (
                        f"Fibonacci SHORT signal strength: {fib_strength:.2f}"
                    )
            ...
            if tails_analysis and tails_analysis.get("signal") == "SHORT":
                ...
                score_breakdown["tails_reason"] = (
                    f"Weekly tails SHORT strength: {tails_strength:.2f}"
                )
            ...
            if trend_analysis and "combined_trend" in trend_analysis:
                ...
                daily_direction = daily_trend.get("direction", "")
                daily_strength = daily_trend.get("strength", "")
                if daily_direction in ["DOWNTREND", "BEARISH"] or daily_strength in [
                    "WEAK",
                    "MODERATE",
                ]:
                ...
                score_breakdown["trend_reason"] = (
                    f"Trend alignment: Daily {daily_direction}({daily_strength}), Weekly {weekly_direction}({weekly_strength})"
                )
            ...
            if divergence_analysis:
                ...
                if (
                    rsi_divergence.get("type") == "bearish"
                    or macd_divergence.get("type") == "bearish"
                ):
            ...
            if ath_proximity_score > 0:
                ...
                score_breakdown["ath_reason"] = (
                    f"ATH proximity bonus: {ath_proximity_score:.2f} (+{ath_score} points)"
                )
                total_score += ath_score
                logger.info(
                    f"ATH proximity bonus added: {ath_score} points (proximity: {ath_proximity_score:.2f})"
                )
            else:
        ...
        try:
            signal_details = {
                ...
                "priority": self._determine_priority(
                    final_signal, fib_analysis, tails_analysis, confluence_info
                ),
                ...
                try:
                    ...
                    if short_candidates:
                        ...
                        strong_short_signals = [
                            s for s in short_signals if s.get("confidence", 0) > 0.7
                        ]
                        ...
                        if strong_short_signals:
                            ...
                            if (
                                ...
                                signal_details["reason"] = best_short.get(
