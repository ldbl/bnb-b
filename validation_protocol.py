...
class ValidationProtocol:
    ...
    def validate_feature(self, feature_name: str, test_periods: Optional[List[str]] = None) -> ValidationResult:
        """
            ...
            try:
                ...
                if result['passed']:
                    ...
                    if validation_point.critical:
                        critical_failures += 1
                    logger.error(f"❌ {validation_point.name}: FAILED - {result.get('message', '')}")
        ...
        summary = self._generate_validation_summary(
            feature_name, passed_points, failed_points, critical_failures, len(self.validation_points)
        )
        ...
        logger.info(f"🛡️ Validation завършена: {passed_points}/{len(self.validation_points)} точки минати")
        if deployment_ready:
    ...
    def _validate_long_accuracy(self, feature_name: str, test_results: Dict[str, TestResult],
                               validation_point: ValidationPoint) -> Dict[str, Any]:
        """
        ...
        if not long_accuracies:
            return {
                'passed': False,
                'message': "Няма LONG accuracy данни за сравнение"
            }
    ...
    def _validate_drawdown_control(self, feature_name: str, test_results: Dict[str, TestResult],
                                  validation_point: ValidationPoint) -> Dict[str, Any]:
        """
        ...
        if not drawdown_changes:
            return {
                'passed': True,
                'message': "Няма drawdown данни за сравнение"
            }
    ...
    def _validate_short_signal_logic(self, feature_name: str, test_results: Dict[str, TestResult],
                                    validation_point: ValidationPoint) -> Dict[str, Any]:
        """
        ...
        if total_signals == 0:
            return {
                'passed': False,
                'message': "Няма генерирани сигнали за анализ"
            }
        ...
        # Анализираме сигналите - fix variable reuse corruption
        ...
        if total_signals == 0:  # Use original total_signals (accumulated count) for signal analysis
        ...
        return {
            'passed': passed,
            'details': {
                'total_signals': total_signals,  # Use original accumulated count for metrics
                'total_periods': total_periods,  # Include period count for clarity
    ...
    def _validate_configuration(self, feature_name: str, test_results: Dict[str, TestResult],
                               validation_point: ValidationPoint) -> Dict[str, Any]:
        """
    ...
    def _validate_edge_cases(self, feature_name: str, test_results: Dict[str, TestResult],
                            validation_point: ValidationPoint) -> Dict[str, Any]:
        """
    ...
    def _validate_performance_impact(self, feature_name: str, test_results: Dict[str, TestResult],
                                    validation_point: ValidationPoint) -> Dict[str, Any]:
        """
    ...
    def _generate_validation_summary(self, feature_name: str, passed: int, failed: int,
                                   critical_failures: int, total: int) -> str:
        """
    ...
    def save_validation_report(self, validation_result: ValidationResult, output_file: str = "validation_report.txt"):
        """
